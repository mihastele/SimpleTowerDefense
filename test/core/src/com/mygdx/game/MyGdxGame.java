...
class StrongTower extends Tower {
    public StrongTower(int x, int y) {
        ...
        setOffsetBodyX(-coreTank.getWidth() / 2);
        setOffsetCoreX(-coreShoot.getWidth() / 2);
        setOffsetBodyY(-coreTank.getHeight() / 2);
        setOffsetCoreY(-coreShoot.getHeight() / 2);
        tCore = new Sprite(coreTank);
        tShoot = new Sprite(coreShoot);
        tShoot.setOrigin(tShoot.getOriginX()+getOffsetCoreY(),tShoot.getOriginY()+getOffsetCoreX());
        tShoot.setOrigin(33 - 12.5f, 23.5f);
        tankBody = new Sprite(bodyOfTank);
...
public class MyGdxGame extends ApplicationAdapter implements InputProcessor {
    ...
    public void handleCoreRotation() {
        ...
        for (Tower t : towers) {
            ...
            if (!(t.getTimer() + 10 >= t.getShootDelay())) {
                ...
                t.tCore.setPosition(t.getY() + t.getOffsetCoreX() + 5, t.getX() + t.getOffsetCoreY() + 5);
                t.tCore.draw(batch);
                ...
                t.tShoot.setPosition(t.getY() + t.getOffsetCoreX() + 5, t.getX() + t.getOffsetCoreY() + 5);
                t.tShoot.draw(batch);
            }
    ...
    public void handleSamples() {
        for (Tower s : samples) {
            this.batch.begin();
            batch.draw(s.tankBody, s.getY() - 25 + s.getOffsetBodyX(), s.getX() - 25 + s.getOffsetBodyY());
            s.tCore.setRotation(90);
            s.tCore.setPosition(s.getY() + s.getOffsetCoreX() + 5 - 25, s.getX() + 5 - 25 + s.getOffsetCoreY());
            s.tCore.draw(batch);
            this.batch.end();
        }
    }
...
//        private int enemyType = 0;
    ...
    public void create() {
        ...
        samples.push(new BasicTower((screenHeight / 5), screenWidth * 9 / 10 + screenWidth / 20 - 25));
        samples.push(new StrongTower((screenHeight * 2 / 5), screenWidth * 9 / 10 + screenWidth / 20 - 25));
